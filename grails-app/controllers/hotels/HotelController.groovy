...
class HotelController {
    ...
    def index(Integer max) {
        ...
        def hotelList = hotelService.list(params)
        respond hotelList, model:[hotelCount: hotelService.count()]
    }

    def search(Integer max) {
        params.max = Math.min(max ?: 10, 100)

        def hotelName = params.name? params.name : ""
        Long countryId = params.country? Long.parseLong(params.country) : null

        def hotelList = hotelService.listByHotelNameAndCountryName(hotelName, countryId, params)

        respond hotelList, model:[hotelCount: hotelList.totalCount]
    }
